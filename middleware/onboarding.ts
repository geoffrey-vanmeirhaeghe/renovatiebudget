export default defineNuxtRouteMiddleware(async (to) => {
  try {
    // Try to use the auth composable
    ...
    console.error('Failed to complete onboarding middleware check:', error)
    // If there's an error accessing auth, skip the middleware
    return
  }
})